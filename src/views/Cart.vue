...
export default {
  ...
  created() {
    ...
    fetch(String(process.env.API_URL) + ":5000/" + "get-cart", options)
      .then((response) => {
  ...
  methods: {
    ...
    cancelItem(product_id, index) {
      ...
      fetch(String(process.env.API_URL) + ":5000/" + "cancel-item", options)
        .then((response) => {
          // console.log(response);
          return response.json();
        })
    ...
    minusQuantity(product_id, index) {
      if (this.products[index][3] > 1) {
        ...
        fetch(String(process.env.API_URL) + ":5000/" + "minus-quantity", options)
          .then((response) => {
            // console.log(response);
            return response.json();
          })
    ...
    plusQuantity(product_id, index) {
      ...
      fetch(String(process.env.API_URL) + ":5000/" + "plus-quantity", options)
        .then((response) => {
          // console.log(response);
          return response.json();
        })
    ...
    async checkout() {
      if (this.$session.has("user")) {
        if (this.products.length > 0) {
          ...
          fetch(String(process.env.API_URL) + ":5000/" + "checkout", options)
            .then((response) => {
              // console.log(response);
              return response.json();
            })
    ...
    cancelAllItems() {
      ...
      fetch(String(process.env.API_URL) + ":5000/" + "cancel-all", options)
        .then((response) => {
          // console.log(response);
          return response.json();
        })
