...
app.config['MAIL_DEFAULT_SENDER'] = str(os.environ.get('MAIL_DEFAULT_SENDER'))
app.config['MAIL_USE_TLS'] = False
app.config['MAIL_USE_SSL'] = True
# Celery configuration
app.config['CELERY_BROKER_URL'] = str(os.environ.get('CELERY_BROKER_URL'))
app.config['CELERY_RESULT_BACKEND'] = str(os.environ.get('CELERY_RESULT_BACKEND'))

#S3 Configuration
app.config['S3_BUCKET'] = str(os.environ.get('S3_BUCKET'))
app.config['S3_ACCESS_KEY'] = str(os.environ.get('S3_ACCESS_KEY'))
app.config['S3_SECRET_KEY'] = str(os.environ.get('S3_SECRET_KEY'))
app.config['S3_SESSION_TOKEN'] = str(os.environ.get('S3_SESSION_TOKEN'))
app.config['S3_UPLOAD_URL'] = str(os.environ.get('S3_UPLOAD_URL'))
...
def upload_file_to_s3(file, bucket_name, acl="public-read"):
    ...
    return "{}{}".format(app.config["S3_UPLOAD_URL"], file.filename)
...
def update_product():
    ...
    new_desc = data[4]
    new_img_path = data[5]
    new_img_path = os.path.join(app.config['S3_UPLOAD_URL'], new_img_path)
...
def update_image():
    try:
        ...
        ## Upload the Image to S3 here
        upload_file_to_s3(image, app.config['S3_BUCKET'])

        ## The line below (should) only works locally
